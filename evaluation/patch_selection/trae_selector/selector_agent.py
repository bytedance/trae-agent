    ...
    if answer.tool_calls and len(answer.tool_calls) > 0:
        for tool_call in answer.tool_calls:
            ...
            if tool_name == "str_replace_based_edit_tool":
                cmd = f"cd /home/swe-bench/tools/ && /home/swe-bench/py312/bin/python3 execute_str_replace_editor.py"
            elif tool_name == "bash":
                cmd = f"cd /home/swe-bench/tools/ && /home/swe-bench/py312/bin/python3 execute_bash.py"
            else:
                tool_message = LLMMessage(role="user", content="The tool name you provided is not in the list. Please choose one from `str_replace_editor` or `bash`!", tool_result=ToolResult(call_id=tool_call_id, name=tool_name, success=False, error="The tool name you provided is not in the list. Please choose one from `str_replace_editor` or `bash`!"))
            ...
            if not all_arguments_valid:
                ...
                tool_message = LLMMessage(role="user", content="Failed call tool. One of the arguments is dict type, you need to check the definition the tool.", tool_result=ToolResult(call_id=tool_call_id, name=tool_name, success=False, error="Failed call tool. One of the arguments is dict type, you need to check the definition the tool."))
                result.append(tool_message)
                continue
            ...
            print(status)
            tool_message = LLMMessage(role="user", content=res_content, tool_result=ToolResult(call_id=tool_call_id, name=tool_name, success=status != "Tool Call Status: -1", result=res_content, error=None if status != "Tool Call Status: -1" else res_content))
            result.append(tool_message)
...
class SelectorAgent:
    def __init__(
        ...
        *,
        ...
        self.sandbox_session.execute(f"git reset --hard HEAD")
        ...
        self.tools: list[Tool] = [
            tools_registry[tool_name](model_provider=llm_config.model_provider.provider) for tool_name in ['bash', 'str_replace_based_edit_tool']
        ]
        self.llm_client = LLMClient(llm_config)
        self.trajectory_recorder: TrajectoryRecorder = TrajectoryRecorder(trajectory_file_name)

        self.initial_messages.append(LLMMessage(role="system", content=build_system_prompt(len(candidate_list))))
    ...
    def run(self):
        ...
        while turn < self.max_turn:
            ...
            self.trajectory_recorder.record_llm_interaction(messages, llm_response, self.llm_config.model_provider.provider, self.llm_config.model, self.tools)
            answer_content = llm_response.content
            print(f"\n### Selector's Answer({turn})\n", answer_content)
            messages: list[LLMMessage] = []
            match = re.search(r'(?:###\s*)?Status:\s*(success|succeed|successfully|successful)\s*\n\s*(?:###\s*)?Result:', answer_content)

            if match:
                print('Match-1:', match.group(1).strip())
                match = re.search(r'(?:###\s*)?Result:\s*(.+?)\s*(?:###\s*)?Analysis:', answer_content)
                if match:
                ...
                messages += parse_tool_response(llm_response, llm_response.finish_reason or "", self.sandbox_session)
